...
class BookingViewSet(viewsets.ModelViewSet):
    ...
    serializer_class = BookingCreateSerializer  # для create
    ...
    def list(self, request):
        ...
        bookings = Booking.objects.filter(user=request.user).prefetch_related("seats", "session")
        data = []
        for b in bookings:
            seats_list = [
    ...
    def create(self, request):
        ...
        if already_booked.exists():
            ...
            for b in already_booked:
                for s in b.seats.all():
                    taken.append({"row_number": s.row_number, "seat_number": s.seat_number})
            return Response(
...
class SessionSeatsView(APIView):
    ...
    def get(self, request, session_id):
        try:
            ...
            return Response({"error": "Сеансне найден"}, status=status.HTTP_404_NOT_FOUND)
